...
public:
    ...
    void add_record(int64_t flops_count, int64_t transcendental_count, int64_t bytes_accessed) {
...
int RealMain(std::string& hlo_module_path, std::string& output_path) {
    ...
    for (xla::HloComputation* comp : hlo_module->computations()) {
        for (xla::HloInstruction* instr : comp->instructions()) {
            ...
            stats_map.at(instr->opcode()).add_record(flops_count, transcendental_count, bytes_accessed);
        }
    }
    // generate vector
    ...
    for (auto op_code: all_op_codes) {
        ...
        feature_vector.push_back(std::isnan(flops_count) ? -1 : flops_count);
        feature_vector.push_back(std::isnan(transcendental_count) ? -1 : transcendental_count);
        feature_vector.push_back(std::isnan(bytes_accessed) ? -1 : bytes_accessed);
    }
...
int main(int argc, char** argv) {
    if (argc != 3) {
        std::cout << "Usage: gen_feature_vector [hlo_module_path] [output_path]" << std::endl;
        return 0;
    }
    std::string hlo_module_path(argv[1]);
    std::string output_path(argv[2]);
    RealMain(hlo_module_path, output_path);
    return 0;
}
